...
bool Si5351::init(uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr)
{
	...
	uint8_t res = HAL_I2C_IsDeviceReady(&hi2c1, i2c_bus_addr << 1, 1, 10);
	if(res == HAL_OK)
	{
...
uint8_t Si5351::si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	uint8_t ret = HAL_I2C_Mem_Write(&hi2c1,(i2c_bus_addr<<1),addr, 1, (uint8_t*)data,bytes,50);
...
{
	uint8_t ret = HAL_I2C_Mem_Write(&hi2c1,(i2c_bus_addr<<1),addr, 1, &data,1,10);
...
{
	uint8_t reg_val = 0;
	HAL_I2C_Mem_Read(&hi2c1,((i2c_bus_addr<<1)|0x01),addr, 1, &reg_val,1,10);
	return reg_val;
}
